...
SAVEABLE_PROPS = [
    "run_fix_geometry", "run_decimate", "run_clean_images",
    "run_clean_unused", "run_resize_textures", "run_export",
    "merge_distance", "recalculate_normals", "fix_manifold",
    "decimate_ratio", "max_texture_size", "resize_mode",
    "output_filename", "output_folder", "export_selected_only",
    "use_draco", "draco_level", "image_format", "image_quality",
]
...
class AIOPT_OT_clean_images(Operator):
    ...
    def images_are_identical(self, img_a, img_b):
        """
        Full pixel comparison between two images that passed the
        fingerprint check. Only called when fingerprints match,
        so this is a rare slow path for confirmation.
        """
    ...
    def execute(self, context):
        ...
        if len(images) < 2:
            self.report({'INFO'}, "Not enough images to compare")
            return {'FINISHED'}
        ...
        no_fingerprint = []
        for img in images:
            fp = self.get_image_fingerprint(img)
            if fp is None:
                no_fingerprint.append(img)
                continue
        ...
        for _fp, group in fingerprint_groups.items():
            ...
            for i, img_a in enumerate(group):
                ...
                for j in range(i + 1, len(group)):
                    ...
                    if self.images_are_identical(img_a, img_b):
        ...
        self.report({'INFO'}, f"Removed {removed} truly identical image(s)")
        return {'FINISHED'}
...
class AIOPT_OT_clean_unused(Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, f"Removed {before - after} unused data block(s)")
        return {'FINISHED'}
...
class AIOPT_OT_resize_textures(Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, f"Resized {resized} texture(s) to max {max_size}px")
        return {'FINISHED'}
...
class AIOPT_OT_export_glb(Operator):
    ...
    def execute(self, context):
        ...
        if os.path.exists(output_path):
            ...
            self.report({'INFO'}, f"Exported: {output_path} ({size_mb:.2f} MB)")
        else:
            self.report({'ERROR'}, "Export may have failed — file not found")
...
class AIOPT_OT_run_all(Operator):
    ...
    def execute(self, context):
        ...
        if props.run_fix_geometry:
            ...
            results.append("geometry fixed")

        # Step 2: Decimate
        if props.run_decimate:
            bpy.ops.ai_optimizer.decimate()
            results.append("decimated")

        # Step 3: Clean duplicate images
        if props.run_clean_images:
            bpy.ops.ai_optimizer.clean_images()
            results.append("images cleaned")

        # Step 4: Clean unused data
        if props.run_clean_unused:
            bpy.ops.ai_optimizer.clean_unused()
            results.append("unused data removed")

        # Step 5: Resize textures
        if props.run_resize_textures:
            bpy.ops.ai_optimizer.resize_textures()
            results.append("textures resized")

        # Step 6: Export
        if props.run_export:
            bpy.ops.ai_optimizer.export_glb()
            results.append("exported")

        self.report({'INFO'}, f"Pipeline complete: {', '.join(results)}")
        return {'FINISHED'}
...
class AIOPT_Properties(PropertyGroup):
    ...
    run_fix_geometry: BoolProperty(
        name="Fix Geometry", default=True,
        description="Fix non-manifold geometry, merge vertices, recalculate normals"
    )
    run_decimate: BoolProperty(
        name="Decimate", default=True,
        description="Reduce polygon count"
    )
    run_clean_images: BoolProperty(
        name="Clean Images", default=True,
        description="Remove duplicate images"
    )
    run_clean_unused: BoolProperty(
        name="Clean Unused", default=True,
        description="Remove unused data blocks"
    )
    run_resize_textures: BoolProperty(
        name="Resize Textures", default=True,
        description="Resize textures to max size"
    )
    run_export: BoolProperty(
        name="Export GLB", default=True,
        description="Export optimized GLB"
    )

    # -- Geometry settings --
    merge_distance: FloatProperty(
        name="Merge Distance", default=0.0001,
        min=0.00001, max=1.0, precision=5,
    ...
    fix_manifold: BoolProperty(
        name="Fix Manifold", default=True,
        description="Attempt to fix non-manifold (holes, open edges)"
    )

    # -- Decimate settings --
    decimate_ratio: FloatProperty(
        name="Ratio", default=0.1,
        min=0.01, max=1.0, step=1, precision=3,
    ...
    output_filename: StringProperty(
        name="Filename", default="optimized_model.glb",
        description="Output filename"
    )
    output_folder: StringProperty(
        name="Folder", default="", subtype='DIR_PATH',
        description="Output folder (blank = same as .blend file)"
    )
...
class AIOPT_PT_main_panel(Panel):
    ...
    def draw(self, context):
        ...
        if meshes:
            ...
            total_images = len([i for i in bpy.data.images
                                if i.type == 'IMAGE' and i.name not in ('Render Result', 'Viewer Node')])
