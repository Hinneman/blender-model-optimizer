...
- **Black texture smears on fragmented-UV meshes after decimation** are eliminated by the new `Dilate UV Gutters` step (see Added). On AI-generated meshes with hundreds of tiny UV islands packed tightly, decimation causes sub-pixel UV drift; before dilation, drifted faces sampled the unpainted gutter between islands and rendered black. After dilation, each island's edge colors bleed a few pixels outward into the gutter so drifted samples land on a sane color.

### Added

- `Dilate UV Gutters` pipeline step and standalone operator. Rasterizes each mesh's UV coverage onto the referenced textures, then bleeds the colored regions outward by N pixels (default 8) into the surrounding unpainted gutter. Uses a 4-directional morphological dilation on the pixel buffer. Runs after `Resize Textures` so the cost scales with the final texture size, and before `Export GLB` so the dilated pixels survive into the GLB. Default on. Configurable `Dilate Pixels` slider in the Textures panel (1–64, default 8). Only modifies textures that are actually sampled by a UV island; unused images and ones with no UV coverage are skipped.
